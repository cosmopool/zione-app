...
void main() async {
  ...
  setUp(() async {
    ...
    port = "80";
    token =
        "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJmcmVzaCI6ZmFsc2UsImlhdCI6MTY0Nzk4MDcwOCwianRpIjoiODVlNjRmNmYtN2UwZC00ZWY3LWFmZWEtOTQxZmI4MTU0YjgwIiwidHlwZSI6ImFjY2VzcyIsInN1YiI6ImthaW8iLCJuYmYiOjE2NDc5ODA3MDh9.WnfLrkGrqr7_G7FktuSCrpreUnfsleWesk-AEwqxY6w";
  ...
  test('should update a ticket when given valid input', () async {
    ...
    // post a new ticket
    Map tk = ticketListStub[1];
    tk['clientName'] = "Freeza pq vc matou o kuririn";
    tk['clientAddress'] = "estou nervoso";
    tk['serviceType'] = "estou nervoso";

    // update the new ticket
    await api.updateContent(tkEndpoint, tk);

    // check that the new ticket has new values
    final fetch = await api.fetchContent(tkEndpoint);
    fetch.fold((l) => null, (r) => ticketList = r);
    final tkToUpdate = ticketList.firstWhere((e) => e['id'] == tk['id']);
    expect(tkToUpdate, tk);
  });
  ...
  test('should return true when given valid input', () async {
    ...
    // update this new ticket
    final res = await api.updateContent(tkEndpoint, ticketList.last);
